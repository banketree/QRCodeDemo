 ...
 * 15/7/24 Description:
 */
public class ResultActivity extends TAppActivity {
	...
	protected void getBundleExtras(Bundle extras) {
		if (extras != null) {
			...
			if (compressedBitmap != null) {
				mBitmap = BitmapFactory.decodeByteArray(compressedBitmap, 0,
	...
	protected void initViewsAndEvents() {
		...
		if (!TStringUtils.isEmpty(mDecodeTime)) {
			sb.append("\n\n扫描时间:\t\t");
			sb.append(mDecodeTime);
		}
		sb.append("\n\n扫描结果:");

		mResultType.setText(sb.toString());
		mResultContent.setText(mResultStr);

		if (null != mBitmap) {
			mResultImage.setImageBitmap(mBitmap);
		}
	}
	...
	private void initView() {
		...
		mResultType = (TextView) findViewById(R.id.result_type);
		mResultContent = (TextView) findViewById(R.id.result_content);
	}
}
