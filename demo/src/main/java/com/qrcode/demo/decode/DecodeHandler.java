...
package com.qrcode.demo.decode;
...
import com.qrcode.demo.CaptureActivity;
import com.qrcode.demo.utils.Constants;
...
public final class DecodeHandler extends Handler {
	...
	private void decode(byte[] data, int width, int height) {
		...
		String zbarStr = mDecodeUtils.decodeWithZbar(rotatedData, size.width,
				size.height, cropRect);
		String zxingStr = mDecodeUtils.decodeWithZxing(rotatedData, size.width,
				size.height, cropRect);
		...
		if (!TextUtils.isEmpty(resultStr)) {
			...
			if (handler != null) {
				Message message = Message.obtain(handler,
						Constants.ID_DECODE_SUCCESS, resultStr);
				Bundle bundle = new Bundle();
				PlanarYUVLuminanceSource source = new PlanarYUVLuminanceSource(
						rotatedData, size.width, size.height, cropRect.left,
